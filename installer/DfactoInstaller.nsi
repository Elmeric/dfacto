; Dfacto.nsi
;
; This script will install Dfacto into a directory that the user selects.
...
; The Dfacto distribution location
!define DFACTO_DIST "F:\Users\Documents\Python\dfacto\dist\dfacto"

; Create the uninstall log file
;!system "dir /b F:\Users\Documents\Python\dfacto\dist\dfacto > F:\Users\Documents\Python\dfacto\dist\dfacto\uninstall.log"
!system 'dir /b "${DFACTO_DIST}" > "${DFACTO_DIST}\uninstall.log"'
...
Section "Dfacto (required)"
  ...
  ; Add files recursively from the Dfacto ditribution directory, keeping their file properties
  File /a /r "${DFACTO_DIST}\*"

  ; Write the installation path into the registry
  WriteRegStr "${REG_ROOT}" "${REG_APP_PATH}" "Install_Location" "$INSTDIR"

  ; Write the uninstall keys for Windows
  WriteRegStr "${REG_ROOT}" "${REG_UNINSTALL_PATH}" "DisplayName" "Dfacto"
  WriteRegStr "${REG_ROOT}" "${REG_UNINSTALL_PATH}" "DisplayIcon" "$INSTDIR\Dfacto.exe,0"
...
Section "Start Menu Shortcuts"
  ...
  CreateShortcut "$SMPROGRAMS\${NAME}\${NAME}.lnk" "$INSTDIR\Dfacto.exe"
...
Section "Uninstall"
  ...
  Delete "$SMPROGRAMS\Dfacto\*.lnk"
  RMDir "$SMPROGRAMS\Dfacto"
