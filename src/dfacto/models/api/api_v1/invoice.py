...
@dataclass()
class InvoiceModel(DFactoModel[crud.CRUDClient, schemas.Client]):
    crud_object: crud.CRUDInvoice = crud.invoice
    schema: Type[schemas.Invoice] = schemas.Invoice

    # TODO:
    # add (create empty invoice) ==> from base.DFactoModel.add
    # create_from_basket
    ...
    def create_from_basket(
        self, client_id: int, clear_basket: bool = True
    ) -> CommandResponse:
        try:
            basket = crud.client.get_basket(self.Session, client_id)
            ...
            if basket is None:
                return CommandResponse(
                    CommandStatus.FAILED,
                    f"CREATE_FROM-BASKET - Basket of client {client_id} not found.",
                )
            if len(basket.items) <= 0:
                return CommandResponse(
                    CommandStatus.REJECTED,
                    f"CREATE_FROM-BASKET - No items in basket of client "
                    f"{basket.client.name}.",
                )

            try:
                inv = self.crud_object.create_from_basket(
                    self.Session, basket=basket, clear_basket=clear_basket
                )
            except crud.CrudError as exc:
                return CommandResponse(
                    CommandStatus.FAILED,
                    f"CREATE_FROM-BASKET - Cannot create invoice from basket of client "
                    f"{basket.client.name}: {exc}",
                )
            else:
...
invoice = InvoiceModel(db.Session)
