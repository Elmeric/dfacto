...
class _LogServer(mp.Process):
    ...
    def configure(self):
        ...
        console_log_lLevel = logging.WARNING

        root = logging.getLogger()
        try:
            file_handler = logging.FileHandler(self.log_file, mode='w')
        except OSError:
            console_log_lLevel = self.log_level
        else:
            file_handler.setLevel(self.log_level)
            file_handler.setFormatter(logging.Formatter(file_logging_format, logging_date_format))
            ...
            if self.log_on_consolde:
                ...
                console_handler.setLevel(console_log_lLevel)
...
class LogConfig(metaclass=Singleton):
    def __init__(
        ...
        self.log_queue = mp.Queue(maxsize=-1)
        self.log_server = _LogServer(self.log_queue, self.log_file, self.log_level, self.log_on_console)
...
def configure_root_logger(log_queue: mp.Queue, log_level: Union[int, str]):
